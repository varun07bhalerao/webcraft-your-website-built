...
export const BusinessTemplate1 = ({ name, logo, pages, content, onNavigate, onGlobalEdit }: TemplateProps) => (
  <div className="font-sans min-h-full bg-white">
    <nav className="bg-[hsl(215,70%,50%)] text-white px-8 py-4 flex items-center justify-between" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
      {logo ? <img src={logo} alt={name} className="h-8 object-contain" /> : <span className="font-bold text-xl">{name}</span>}
    ...
    <div className="bg-[hsl(215,70%,95%)] px-8 py-20 text-center">
      ...
      <button className="mt-6 bg-[hsl(215,70%,50%)] text-white px-8 py-3 rounded-lg font-semibold" onClick={(e) => { e.preventDefault(); if (content.links?.home_cta) onNavigate?.(content.links.home_cta); }}>{content.home?.cta}</button>
    </div>
    ...
    <footer className="bg-[hsl(215,70%,15%)] text-white px-8 py-10 text-center" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
...
export const BusinessTemplate2 = ({ name, logo, pages, content, onNavigate, onGlobalEdit }: TemplateProps) => (
  <div className="font-sans min-h-full bg-white">
    <header className="bg-slate-900 text-white px-8 py-4 flex items-center justify-between sticky top-0 z-10" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
    ...
    <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-8 py-32 text-white">
      <div className="max-w-4xl">
        ...
        <button className="mt-10 bg-white text-blue-700 px-8 py-4 rounded-md font-bold hover:bg-opacity-90 transition-all" onClick={(e) => { e.preventDefault(); if (content.links?.home_cta) onNavigate?.(content.links.home_cta); }}>{content.home?.cta}</button>
      </div>
    </div>
    ...
    <footer className="bg-slate-900 text-white p-12 text-center md:text-left flex flex-col md:flex-row justify-between items-center border-t border-slate-800" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
      <div>
...
export const BusinessTemplate3 = ({ name, logo, pages, content, onNavigate, onGlobalEdit }: TemplateProps) => (
  <div className="font-sans min-h-full bg-[#0f172a] text-slate-200">
    <nav className="bg-[#1e293b]/80 backdrop-blur-md px-8 py-4 flex items-center justify-between sticky top-0 z-10 border-b border-slate-800" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
    ...
    <div className="px-8 py-32 text-center bg-[radial-gradient(circle_at_50%_0%,#1e293b,transparent_70%)]">
      ...
      <div className="mt-12 flex gap-4 justify-center">
        <button className="bg-blue-600 text-white px-8 py-3 rounded-full font-bold" onClick={(e) => { e.preventDefault(); if (content.links?.home_cta) onNavigate?.(content.links.home_cta); }}>{content.home?.cta}</button>
    ...
    <footer className="border-t border-slate-800 py-20 px-8 text-center bg-[#070b14]" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
...
export const BusinessTemplate4 = ({ name, logo, pages, content, onNavigate, onGlobalEdit }: TemplateProps) => (
  <div className="font-sans min-h-full bg-[#fffbeb]">
    <div className="bg-[#f59e0b] h-2 w-full" />
    <nav className="px-8 py-6 flex flex-col md:flex-row items-center justify-between gap-4" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
      ...
      <div className="bg-white rounded-3xl overflow-hidden shadow-xl border border-[#fef3c7]">
        <div className="grid md:grid-cols-2">
          <div className="p-12 md:p-20 flex flex-col justify-center bg-white">
            ...
            <button className="mt-10 bg-[#f59e0b] text-white px-8 py-4 rounded-2xl font-bold shadow-lg shadow-orange-500/20 self-start" onClick={(e) => { e.preventDefault(); if (content.links?.home_cta) onNavigate?.(content.links.home_cta); }}>{content.home?.cta}</button>
          </div>
    ...
    <footer className="bg-[#451a03] text-white px-8 py-16" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
...
export const BusinessTemplate5 = ({ name, logo, pages, content, onNavigate, onGlobalEdit }: TemplateProps) => (
  <div className="font-serif min-h-full bg-white text-black">
    <header className="border-b border-black/10 px-12 py-8 flex items-center justify-between uppercase tracking-[0.2em] text-xs font-medium" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
    ...
    <footer className="bg-black text-white p-20 flex flex-col items-center text-center" onClick={(e) => { e.stopPropagation(); onGlobalEdit?.('contact'); }}>
